...
for section in fit_sections:
    ...
    if 'el' in codename:
      ...
      raise RuntimeError('invalid channel name: ' + section)
    if 'WW' in codename:
      cat = 'WW'
    elif 'WZ' in codename:
      cat = 'WZ'
    ...
    old_obs.SetName("mlvj")
    old_obs2 = oldWS.var('mj_%s'%codename[3:-3])

    print codename
    old_obs.Print()
    old_obs2.Print()

    getattr(theWS,'import')(old_obs)
    getattr(theWS,'import')(old_obs2)

    data	= RooDataSet('data_obs','data_obs',oldWS.data('dataset_mj_%s'%codename[3:]),RooArgSet(old_obs,old_obs2))
    data.Print()
...
##FIXME TTbar pdf has to be decorrelated to apply shape parameter errors here
##FIXME add correct parameter names
...
#rrv_c_ErfExp_WJets0_el flatParam
#rrv_c_ErfExp_WJets0_mu flatParam
...
cmd = 'combineCards.py aC_WW_sig_el.txt aC_WZ_sig_el.txt aC_WW_sb_lo_el.txt aC_WZ_sb_lo_el.txt aC_WW_sb_hi_el.txt aC_WZ_sb_hi_el.txt aC_WW_sig_mu.txt aC_WZ_sig_mu.txt aC_WW_sb_lo_mu.txt aC_WZ_sb_lo_mu.txt aC_WW_sb_hi_mu.txt aC_WZ_sb_hi_mu.txt > %s'%(options.ocard)
print cmd
os.system(cmd)
